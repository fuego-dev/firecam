...
from lib.img_archive import parseFilename
...
def test_set_generator(cropped_image_list, image_name_list, test_size, n=5):
    ...
    for i in range(test_size):
        ...
        neighbors = [index for index, image in enumerate(image_name_list) if
                     image["cameraID"] == drawn_name and image["date"] == drawn_date and float(
                         image["hours"]) * 60 + float(image["minutes"]) >= minute_time - 2 * n and float(
                         image["hours"]) * 60 + float(image["minutes"]) <= minute_time + n // 2]
...
def main():
    opt_args = [
        ["i", "inputDir", "Input directory where your Cropped_Images.csv is found"],
        ["o", "outputDir", "directory to save the output image"],
    ]
    args = collect_args.collectArgs([], optionalArgs=opt_args)

    if args.inputDir:
        input_dir = args.inputDir
    else:
        input_dir = '/home/fuego/Desktop/'
    ...
    input_csv = input_dir + 'Cropped_Images.csv'  # Replace with path to your csv
    cropped_image_list, image_name_list = image_library(input_csv)
    training_set, test_set = test_set_generator(cropped_image_list, image_name_list,
                                                100)  # adjust value for desired test set size
    save_new_sets(training_set, test_set, output_dir)  # change output directory to your desired location
