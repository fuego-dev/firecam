...
def holdLeftClick(event):
    ...
    chaine.configure(text=' '.join(list(map(lambda x: str(int(x)), [x1, y1, x2, y2]))) + " Frame object number " + str(
        len(rectangleShapes)))
...
def releaseLeftClick(event):
    ...
    if (minX == maxX) or (minY == maxY):
        ...
        return
    chaine.configure(
        text=' '.join(list(map(lambda x: str(int(x)), area))) + " Number of frames " + str(len(rectangleShapes) + 1))
