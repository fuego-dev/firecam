...
def build_name_with_metadata(image_base_name, metadata):
    ...
    metadata_chunk = 'p' + str(metadata['position']['pan']) + '_t' + str(metadata['position']['tilt']) + '_z' + str(
        metadata['position']['zoom'])
...
def capture_and_record(googleServices, dbManager, outputDir, camera_name):
    ...
    img_archive.addImageToArchiveDb(dbManager, camera_name, timeStamp, 'gs://' + settings.archive_storage_bucket,
                                    cloud_file_path, pull1['position']['pan'], pull1['position']['tilt'],
    ...
    if args.agents:
        ...
        if num_of_processes_needed > agents:
            logging.warning(
                ...
                agents, target_refresh_time_per_camera, num_cameras_per_process * agents, num_of_processes_needed)
            return
        ...
        if args.full_system:
            ...
            listofTargetCameras["rotating"] = [camera["name"] for camera in listofCameras if
                                               (camera["name"][-1] == '2')]
            listofTargetCameras["stationary"] = [camera["name"] for camera in listofCameras if
                                                 (camera["name"][-1] != '2')]
            camera_bunchs = []
            for type in ["rotating", "stationary"]:
                num_cameras_per_process[type] = math.floor(
                    target_refresh_time_per_camera[type] / response_time_per_camera)
                # divy up cameras rotating and stationary to maximize efficiency
                num_of_processes_needed[type] = math.ceil(
                    len(listofTargetCameras[type]) / num_cameras_per_process[type])
                num_cameras_per_process[type] = math.floor(
                    len(listofTargetCameras[type]) / num_of_processes_needed[type])
